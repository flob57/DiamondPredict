...
async function getJSON(url,options={}){const r=await fetch(url,{cache:'no-store',...options});const body=await r.json().catch(()=>({}));if(!r.ok)throw new Error(body.error||`HTTP ${r.status}`);return body}
...
export async function fetchSync(syncKey){return getJSON(new URL('/api/sync',location.origin),{headers:{'x-diamond-key':syncKey}})}
export async function saveSync(syncKey,state){const r=await fetch(new URL('/api/sync',location.origin),{method:'POST',headers:{'content-type':'application/json','x-diamond-key':syncKey},body:JSON.stringify(state)});const body=await r.json().catch(()=>({}));if(!r.ok)throw new Error(body.error||`HTTP ${r.status}`);return body}
